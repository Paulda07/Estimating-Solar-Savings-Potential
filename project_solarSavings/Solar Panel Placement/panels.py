...
if __name__ == "__main__":
    ...
    parameters = {
        "format" : "json",
        "api_key":"zourhmnG6T4yL9qjTD09rNzJMDuaCj8BELcEfgHg",
        ...
        "lat": 41.901858,
        "lon":  -88.091047,
        "address": "Glendale Heights, IL, USA"
    }
    lat = "17.425214"
    lon = "78.542519"
    response = requests.get("https://developer.nrel.gov/api/pvwatts/v6.json?api_key=fSOibWSTKTlii210ZqBoyfIAYjA3lhjQCrq4y2Vr&lat="+lat+"&lon="+lon+"&system_capacity=4&azimuth=180&tilt=40&array_type=1&module_type=1&losses=10")
    print(response.status_code)
    ir = response.json()["outputs"]["solrad_annual"]
#/api/alt-fuel-stations/v1.json?fuel_type=E85,ELEC&state=CA&limit=2&api_key=BzuAupefUPNHhzETjW3SucqtcPWeEVANUuNZtpq7&format=JSON
#response = requests.get("/api/pvwatts/v6.json?api_key=zourhmnG6T4yL9qjTD09rNzJMDuaCj8BELcEfgHg&lat=40&lon=-105&system_capacity=4&azimuth=180&tilt=40&array_type=1&module_type=1&losses=10")
#https://developer.nrel.gov/
#https://solarrooftop.gov.in/rooftop_calculator

    images = glob.glob('testcases/30.jpg')
    # latitude = ??
    ...
    for fname in images:
        ...
        plt.show()
        a = (n_white_pix/1000)*0.9
        print (a)
        energy = ir*a*0.7
        print("Energy produced per annum: "+str(energy))
